...
describe("Typed Starknet Signer", () => {
  ...
  it("should verify a known values", async () => {
    const data = Buffer.from("Hello-world!");
    const signature = await signer.sign(data);
    const isValid = await InjectedStarknet.verify(signer.publicKey, data, signature);
  ...
  it("should sign & verify an unknown value", async () => {
    ...
    const isValid = await InjectedStarknet.verify(signer.publicKey, randData, signature);
    expect(isValid).toEqual(true);
  });
